...
public:
    bool isValid(vector<vector<int>>& grid, int row, int col, int n,int expectVal){
        if(row>=n || row<0 || col>=n || col<0 || grid[row][col]!=expectVal) return false;
        ...
        int ans5=isValid(grid,row+2,col-1,n,expectVal+1);
        int ans6=isValid(grid,row+1,col-2,n,expectVal+1);
